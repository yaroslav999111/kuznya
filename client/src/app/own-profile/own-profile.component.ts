...
export class OwnProfileComponent implements OnInit {
  ...
  constructor(public loginService: LoginService, public projectsService: ProjectsService, public userService: UsersService) { }
  ...
  getUserProfile() {
   let userId = localStorage.getItem('userEmail');
    this.loginService.getUserDetailsById(userId).subscribe(data => {
      ...
      switch (this.profileInformation.position) {
        case '0': this.userService.positionInCompany.next({positionInCompany: 'Режиссер-постановщик'});
          break;
        case '1': this.userService.positionInCompany.next({positionInCompany: '2й режиссер'});
          break;
      }

      this.positionInCompany = this.userService.positionInCompany.value['positionInCompany'];
